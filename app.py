...
class Station(db.Model):
    ...
    def __repr__(self):
        return "<Station {id} ({lat}, {lon})>".format(
            id=self.id, lat=self.latitude, lon=self.longitude
        )

    def get_stations_within_radius(self, radius):
        """Return all stations within a given radius (in meters)."""
        return Station.query.filter(
            func.ST_Distance_Sphere(Station.geometry, self.geometry) < radius
        ).all()
    ...
    def update_geometries(cls):
        """Using each station's longitude and latitude, add geometry data to db."""
        stations = Station.query.all()
        for station in stations:
            point = "POINT({} {})".format(station.longitude, station.latitude)
...
def create_a_station():
    ...
    new_station = Station(
        ...
        geometry="POINT({} {})".format(data.get("longitude"), data.get("latitude")),
...
def update_station():
    ...
    updating_station.geometry = "POINT({} {})".format(
        updating_station.longitude, updating_station.latitude
    )
    db.session.commit()
    serializer = StationSchema()
